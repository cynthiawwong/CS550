...
def dolphin_classifier(data_directory):
    ...
    #separate paths for dolphin data for ease of loading data
    directories = []
    directories.append(data_directory + "/Gg")
    directories.append(data_directory + "/Lo")
    ...
    for func in split_functions: 
        ...
        print("Model for {}".format(func.__name__))

        #split data by specified function
        train, test = get_data(directories,use_onlyN, func) 
        ...
        #optimization: stochastic gradient descent: SGD, batch gradient descent, Adam 
        model.compile(optimizer = "Adam", loss = "categorical_crossentropy", metrics = ["accuracy"])

        Y_onehot_train = to_categorical(Y_train) #one-hot labels

        #train the model
        model.fit(X_train, Y_onehot_train, batch_size=100, epochs=10, class_weight=class_weight, shuffle=True)
        
        #predict the model
        Y_pred = model.predict(X_test)
        
        #group probabilites in groups of 100 and leave out the last group if it is an incomplet set 
        cycles = int(len(Y_pred)/100)*100 
        ...
        for i in range(0,cycles,100): 
            ...
            joint_prob = np.sum(Y_pred[i:i+99],axis=0) #sum the log of 100 predicted labels
        ...
        #plot confusion matrix and calculate error rate 
...
def get_data(directories, use_onlyN, split_function): 
    """
    ...
    :param directories:  A list of directories for the data, one for Risso's dolphin and one for Pacific white-side dolphin
    ...
    Risso_files = util.get_files(directories[0], stop_after = use_onlyN)
    ...
    Risso_split_train, Risso_split_test = train_test_split(Risso_split,random_state=17) #split data into training and test data 
    
    #get Pacific white-sided dolphin clicks and split into training and test data
    Pacific_files = util.get_files(directories[1], stop_after = use_onlyN)
    ...
    Pacific_split_train, Pacific_split_test = train_test_split(Pacific_split,random_state=20) #split data into training and test data 
...
def compute_class_weight(Y_train):
    ...
    Risso = len(Y_train)-np.sum(Y_train) #no. of Risso data
    Pacific = np.sum(Y_train) #no. of Pacific data
...
def confusion_matrix_plot(Y_test, Y_pred, func): 
    ...
    plt.xlabel('Predicted Class\naccuracy={:.2%}; error rate={:.2%}'.format(accuracy, error_rate))
...
if __name__ == "__main__":
    data_directory = "/Users/wwe/Documents/SDSU Classes/Senior/Fall 2021/CS550 - Artificial Intelligence/a4/features" #"path\to\data"  # root directory of data
